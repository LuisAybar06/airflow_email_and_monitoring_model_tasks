...
def task_monitoringmodel():
    ...
    avg_house_price = np.mean(exp_y_train)
    ...
    feature_importance = pd.DataFrame({
        'Processing_Date': [processing_date] * len(features),
...
def task_monitoringmodel_v2():
    ...
    engine = create_engine('mysql+mysqlconnector://root:tlfJEHTHSaWMrjaRMVHHrmMHHwJtYHSQ@roundhouse.proxy.rlwy.net:25557/railway')

    # Cargar datos
    X_train = pd.read_csv('/opt/airflow/dags/data/inputs/xtrain.csv')
    y_train = pd.read_csv('/opt/airflow/dags/data/inputs/ytrain.csv')
    X_test = pd.read_csv('/opt/airflow/dags/data/inputs/xtest.csv')
    y_test = pd.read_sql('SELECT SalePrice FROM ytest', con=engine)
    ...
    feature_importance = pd.DataFrame({
        'Processing_Date': [processing_date] * len(features),
        'Feature': importance.index.tolist(),
        'Importance': importance.values.tolist()
    })


    # Guardar los DataFrames en las tablas MySQL
    metrics_results.to_sql('model_evaluation_metrics_results', con=engine, if_exists='append', index=False)
    feature_importance.to_sql('model_evaluation_feature_importance', con=engine, if_exists='append', index=False)